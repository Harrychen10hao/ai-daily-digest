...
- 飞书消息使用群机器人支持的 `interactive` 交互卡片。
- 早报标题、日期、重点新闻和分类层级清晰可读。
- 每条新闻显示 emoji、Markdown 粗体标题、摘要、价值说明和可点击原文链接。
...
消息中不显示 Markdown 的 `#`、`##`、`###` 标记；卡片 Markdown 只用于飞书支持的粗体和链接。
...
`send_pipeline` 优先读取 `latest_digest.json`，通过新的飞书交互卡片格式化器生成消息 payload。若结构化文件不存在，给出要求重新执行 `generate` 的清晰错误，不从 Markdown 反向解析。

### 飞书 payload

新增卡片格式化函数，将一个 digest 转换为一个或多个飞书 `interactive` 消息 payload：
...
```json
{
  ...
  "card": {
    ...
    "elements": [
      {"tag": "div", "text": {"tag": "lark_md", "content": "**① 新闻标题**\\n摘要：...\\n🔗 [查看原文](https://example.com/article)"}},
      {"tag": "hr"}
    ]
  }
}
...
现有 `FeishuClient.send` 的普通文本能力保留；新增通用结构化 payload 发送入口，`send_pipeline` 使用交互卡片入口。卡片中的 Markdown 只使用飞书支持的粗体、链接和分隔线。
...
- 卡片构造时缺少 URL 的条目直接省略链接行，不编造 URL。
...
- 卡片格式化测试：验证标题层级、重点编号、加粗标题、emoji、可点击链接、分割线和空分类省略。
- 拆分测试：验证按完整条目拆分、最大长度、序号和无空消息。
- 飞书客户端测试：使用 `httpx.MockTransport` 验证 `msg_type=interactive`、请求 payload、重试和业务错误处理。
